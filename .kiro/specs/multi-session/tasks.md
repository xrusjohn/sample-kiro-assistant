...
- [x] 0a. Create `src/server/pid-tracker.ts` — track child PIDs in `/tmp/kiro-assistant-pids.json`, with add/remove/killStale/cleanup methods
- [x] 0b. Integrate pid-tracker into `createAcpRunner` (register on spawn, unregister on child exit)
- [x] 0c. Add graceful shutdown handler in `src/server/index.ts` — catch SIGINT/SIGTERM, abort all runners, clean PID file, exit
- [x] 0d. Call `killStale()` on server boot before starting the HTTP listener

## Phase 1: Runner Lifecycle Manager

- [x] 1. Create `src/server/runner-manager.ts` with `RunnerManager` class
...
- [x] 4. Refactor `src/server/session-handler.ts` to use `RunnerManager` instead of raw `runnerHandles` Map
...
- [x] 5. Read `KIRO_MAX_SESSIONS` and `KIRO_IDLE_TIMEOUT_MINUTES` env vars in session-handler or runner-manager, with defaults (5 and 30)

- [x] 6. Add `GET /api/sessions/health` endpoint in `src/server/index.ts` returning active process count and per-session state
...
- [x] 8. Update `src/ui/components/Sidebar.tsx` to show status indicators per session
